...
var FFmpeg = class {
	...
	*/
	load = ({ classWorkerURL, ...config } = {}, { signal } = {}) => {
		if (!this.#worker) {
			this.#worker = classWorkerURL ? new Worker(new URL(classWorkerURL, import.meta.url), { type: "module" }) : new Worker(new URL("./worker.js", import.meta.url), { type: "module" });
...
//#endregion
//#region src/utils/load_ffmpeg.ts
function toAssetURL(assetPath) {
	return new URL(assetPath, window.location.href).toString();
}
var ffmpegAssetUrlsPromise = null;
async function loadFFmpeg() {
	if (ffmpegAssetUrlsPromise === null) ffmpegAssetUrlsPromise = Promise.all([import("./ffmpeg-core-CaI_P3fo.js"), import("./ffmpeg-core-Cc_Mbord.js")]).then(([coreModule, wasmModule]) => {
		return {
