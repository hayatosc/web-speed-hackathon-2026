...
export async function insertSeeds(db: DrizzleDB) {
    ...
    await db.insert(schema.users).values(
      await Promise.all(
        batch.map(async (user) => ({
          ...user,
          password: await normalizeStoredPassword(user.password),
        }))
      )
    );
  });
  ...
  await readJsonlFileBatched<DirectMessageConversationSeed>('directMessageConversations.jsonl', async (batch) => {
